...
describe('registerFunction', () => {
  ...
  it('alerts too few arguments', () => {
    jmespath.registerFunction('tooFewArgs', () => {
      /* EMPTY FUNCTION */
      return 'empty';
  ...
  it('alerts too many arguments', () => {
    jmespath.registerFunction('tooManyArgs', () => {
      /* EMPTY FUNCTION */
      return 'empty';
    }, []);
    expect(() =>
  ...
  it('alerts optional variadic arguments', () => {
    jmespath.registerFunction('optionalVariadic', () => {
      /* EMPTY FUNCTION */
      return 'empty';
  ...
  it('alerts variadic is always last argument', () => {
    expect(() =>
      jmespath.registerFunction('variadicAlwaysLast', () => {
        /* EMPTY FUNCTION */
        return 'empty';
      }, [
...
describe('root', () => {
  it('$ should give access to the root value', () => {
    const value = <JSONObject>jmespath.search({ foo: { bar: 1 } }, 'foo.{ value: $.foo.bar }');
    expect(value).not.toBe(null);
    expect(value?.['value']).toBe(1);
  });
  it('$ should give access to the root value after pipe', () => {
    const value = jmespath.search({ foo: { bar: 1 } }, 'foo | $.foo.bar');
    expect(value).toEqual(1);
  });
  it('$ should give access in expressions', () => {
    const value = jmespath.search([{ foo: { bar: 1 } }, { foo: { bar: 99 } }], 'map(&foo.{boo: bar, root: $}, @)');
  ...
  it('$ can be used in parallel', () => {
    const value = jmespath.search([{ foo: { bar: 1 } }, { foo: { bar: 99 } }], '[$[0].foo.bar, $[1].foo.bar]');
...
describe('Enhanced Registry API', () => {
  ...
  describe('register() function', () => {
    it('should register a new function successfully', () => {
      const result = jmespath.register('multiply', ([a, b]) => (a as number) * (b as number), [
      ...
      expect(jmespath.isRegistered('multiply')).toBe(true);
    });
    ...
    it('should prevent registering built-in functions at compile time', () => {
      ...
      // jmespath.register('sum', ([a, b]) => a + b, [{ types: [jmespath.TYPE_NUMBER] }]);

      // We can test runtime behavior by bypassing TypeScript
      // biome-ignore lint/suspicious/noExplicitAny: Testing runtime behavior
      const result = (jmespath.register as (name: string, func: any, sig: any) => RegistrationResult)(
        'sum',
        // biome-ignore lint/suspicious/noExplicitAny: Testing runtime behavior
    ...
    it('should return error when function already exists without override', () => {
      jmespath.register('testFunc', () => 'first', []);
      const result = jmespath.register('testFunc', () => 'second', []);
    ...
    it('should allow overriding with override option', () => {
      jmespath.register('testFunc', () => 'first', []);
      const result = jmespath.register('testFunc', () => 'second', [], { override: true });
    ...
    it('should emit warning when overriding with warn option', () => {
      ...
      jmespath.register('testFunc', () => 'first', []);
      jmespath.register('testFunc', () => 'second', [], { override: true, warn: true });
    ...
    it('should validate function name', () => {
      const result = jmespath.register('', () => 'test', []);
      expect(result.success).toBe(false);
      if (!result.success) {
    ...
    it('should validate function signature', () => {
      const result = jmespath.register('testFunc', () => 'test', [
        { types: [jmespath.TYPE_ANY], variadic: true },
  ...
  describe('registerFunction() backward compatibility', () => {
    it('should maintain backward compatibility', () => {
      expect(() => {
        jmespath.registerFunction('oldStyle', ([a, b]) => (a as number) + (b as number), [
          { types: [jmespath.TYPE_NUMBER] },
          { types: [jmespath.TYPE_NUMBER] },
        ]);
      }).not.toThrow();

      expect(jmespath.search({ a: 5, b: 3 }, 'oldStyle(a, b)')).toBe(8);
    });

    it('should support new options in registerFunction', () => {
      jmespath.registerFunction('testFunc', () => 'first', []);

      expect(() => {
        jmespath.registerFunction('testFunc', () => 'second', [], { override: true });
      }).not.toThrow();
    });
    ...
    it('should throw error for duplicate registration without override', () => {
      ...
      expect(() => {
        jmespath.registerFunction('testFunc', () => 'second', []);
  ...
  describe('unregisterFunction()', () => {
    it('should unregister custom functions', () => {
      jmespath.register('testFunc', () => 'test', []);
      expect(jmespath.isRegistered('testFunc')).toBe(true);

      const result = jmespath.unregisterFunction('testFunc');
      expect(result).toBe(true);
      expect(jmespath.isRegistered('testFunc')).toBe(false);
    });
    ...
    it('should not unregister built-in functions', () => {
      ...
      // biome-ignore lint/suspicious/noExplicitAny: Testing runtime behavior
      const result = (jmespath.unregisterFunction as any)('sum');
      expect(result).toBe(false);
      expect(jmespath.isRegistered('sum')).toBe(true);
    });

    it('should return false for non-existent functions', () => {
      const result = jmespath.unregisterFunction('nonExistent');
      expect(result).toBe(false);
    });
  });
  ...
  describe('isRegistered()', () => {
    it('should return true for built-in functions', () => {
      expect(jmespath.isRegistered('sum')).toBe(true);
      expect(jmespath.isRegistered('length')).toBe(true);
      expect(jmespath.isRegistered('map')).toBe(true);
    });

    it('should return true for custom functions', () => {
      jmespath.register('customFunc', () => 'test', []);
      expect(jmespath.isRegistered('customFunc')).toBe(true);
    });

    it('should return false for non-existent functions', () => {
      expect(jmespath.isRegistered('nonExistent')).toBe(false);
    });
  });

  describe('getRegisteredFunctions()', () => {
    it('should return all registered functions including built-ins', () => {
      const functions = jmespath.getRegisteredFunctions();
    ...
    it('should include custom functions', () => {
      jmespath.register('customFunc1', () => 'test1', []);
      jmespath.register('customFunc2', () => 'test2', []);
  ...
  describe('getCustomFunctions()', () => {
    it('should return only custom functions', () => {
      const customFunctions = jmespath.getCustomFunctions();
      expect(customFunctions).toEqual([]);

      jmespath.register('customFunc1', () => 'test1', []);
      jmespath.register('customFunc2', () => 'test2', []);

      const updatedCustomFunctions = jmespath.getCustomFunctions();
  ...
  describe('clearCustomFunctions()', () => {
    it('should remove all custom functions but keep built-ins', () => {
      ...
      expect(jmespath.getCustomFunctions()).toHaveLength(2);
      expect(jmespath.isRegistered('sum')).toBe(true);

      jmespath.clearCustomFunctions();

      expect(jmespath.getCustomFunctions()).toHaveLength(0);
      expect(jmespath.isRegistered('customFunc1')).toBe(false);
      expect(jmespath.isRegistered('customFunc2')).toBe(false);
      expect(jmespath.isRegistered('sum')).toBe(true); // Built-in should remain
    });
  });
});
